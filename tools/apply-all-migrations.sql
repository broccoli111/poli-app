-- ============================================================
-- Poli App: Complete Phase 1 Migration
-- Run this in the Supabase SQL Editor (Dashboard > SQL Editor)
--
-- BEFORE running this:
-- 1. Go to Database > Extensions and enable: uuid-ossp
-- 2. Go to Storage and create a bucket called "bill-text" (private)
--
-- AFTER running this, optionally enable:
-- 3. Database > Extensions: pg_cron, pg_net (for nightly pipeline)
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_bills_external ON bills(external_id);
...
CREATE INDEX IF NOT EXISTS idx_btv_bill ON bill_text_versions(bill_id);
...
CREATE INDEX IF NOT EXISTS idx_bc_bill ON bill_categories(bill_id);
CREATE INDEX IF NOT EXISTS idx_bc_category ON bill_categories(category);
...
CREATE INDEX IF NOT EXISTS idx_ai_bill ON ai_summaries(bill_id);
...
CREATE INDEX IF NOT EXISTS idx_bvm_bill ON bill_vote_member(bill_id);
CREATE INDEX IF NOT EXISTS idx_bvm_politician ON bill_vote_member(politician_id);
...
CREATE INDEX IF NOT EXISTS idx_ubs_user ON user_bill_sentiment(user_id);
CREATE INDEX IF NOT EXISTS idx_ubs_bill ON user_bill_sentiment(bill_id);
...
CREATE INDEX IF NOT EXISTS idx_hs_politician ON honesty_scores(politician_id);
...
CREATE INDEX IF NOT EXISTS idx_pf_politician ON politician_funding(politician_id);
...
-- =====================
-- SEED DATA (before RLS so inserts work)
-- =====================
...
-- Authenticated users can write sentiment aggregates
...
-- (Seed data inserted above, before RLS was enabled)
